...
function proxy(req, client, buffer, hosts) {
  ...
  // 匹配额外的hosts文件
  var host = hosts.resolve(req.host) || req.host;

  //建立到目标服务器的连接
  var server = net.createConnection(req.port, host);
  //交换服务器与浏览器的数据
  client.pipe(server);
  server.pipe(client);
  // 异常
